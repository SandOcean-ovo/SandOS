...
### 1. 双架构对比实现
项目中同时实现了 ARM Cortex-M3 和 RISC-V 两种架构的上下文切换，可直观对比：
- **ARM**：利用硬件自动压栈（xPSR, PC, LR, R12, R3-R0），减少切换开销
- **RISC-V**：软件全手动保存（x1, x3-x31 + CSR），深入理解寄存器操作

这种对比实现能让你在面试中清晰讲述**不同体系架构的 trap 处理机制**。

### 2. 位图调度算法
采用 `g_PrioMap` 位图 + `OS_MapTable` 查表法，实现真正的 O(1) 优先级查找，而非常见的遍历方案。

### 3. 优先级继承
互斥锁实现了完整的优先级继承机制，能现场手写代码讲解**优先级翻转问题**及其解决方案。

### 4. 静态内存池
利用空闲块头部存储指针实现零额外开销的链表，无动态内存碎片，面试时可与 `malloc`/堆管理对比讲解。
